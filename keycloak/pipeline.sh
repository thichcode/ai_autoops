...
cleanup() { local rc=$?; if [[ $rc -ne 0 && "$ROLLBACK_TRIGGERED" == false ]]; then ROLLBACK_TRIGGERED=true; notify_failure "Keycloak pipeline failed at $CURRENT_STAGE" ""; run_stage "rollback"; fi; audit_record "PIPELINE_END" "$([ $rc -eq 0 ] && echo SUCCESS || echo FAILURE)" "Stages: ${COMPLETED_STAGES[*]}"; }; trap cleanup EXIT
...
audit_started "PIPELINE" "Keycloak $ACTION on $ENV"; notify "Keycloak pipeline started"
run_stage "precheck" && run_stage "backup" && run_stage "restore-test"
if [[ "$ACTION" == "patch" ]]; then
    for node in "${KC_NODES[@]}"; do export CURRENT_NODE="$node"; run_stage "drain" && run_stage "maint" && run_stage "deploy" && run_stage "verify" || { run_stage "rollback"; exit 1; }; done
fi
notify_success "Keycloak pipeline completed"
