...
audit_started "DRAIN" "Draining AWX LB"
local NETRC=$(mktemp); echo "default login $NGINX_PLUS_API_USER password $NGINX_PLUS_API_PASS" > "$NETRC"; chmod 600 "$NETRC"
curl -s -X PATCH -d '{"weight": 0}' "$NGINX_PLUS_API/6/upstreams/$NGINX_UPSTREAM_NAME/servers/$AWX_NODE" --netrc-file "$NETRC" || true
rm -f "$NETRC"; sleep 10; audit_success "DRAIN" "AWX drained"
