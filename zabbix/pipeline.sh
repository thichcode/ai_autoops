...
cleanup() { local rc=$?; if [[ $rc -ne 0 && "$ROLLBACK_TRIGGERED" == false ]]; then ROLLBACK_TRIGGERED=true; notify_failure "Zabbix pipeline failed at $CURRENT_STAGE" ""; run_stage "rollback"; fi; audit_record "PIPELINE_END" "$([ $rc -eq 0 ] && echo SUCCESS || echo FAILURE)" "Stages: ${COMPLETED_STAGES[*]}"; }
trap cleanup EXIT
...
audit_started "PIPELINE" "Zabbix $ACTION on $ENV"; notify "Zabbix pipeline started" "Action: $ACTION, Env: $ENV"
...
notify_success "Zabbix pipeline completed"
