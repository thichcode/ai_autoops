#!/bin/bash
# restore-test.sh — Restore BlackDuck from Backup Exec to staging & verify
set -euo pipefail
...
audit_started "RESTORE_TEST" "Restoring BlackDuck to staging"

source "$SCRIPT_DIR/config/staging.cfg"
STG_HOST="$BD_HOST"

REPORT_DIR="${BACKUP_BASE}/reports"
REPORT_FILE="${REPORT_DIR}/restore-test-blackduck-${ENV}-$(date +%Y%m%d-%H%M%S).md"
mkdir -p "$REPORT_DIR"

TEST_ID="RESTORE-BD-$(date +%Y%m%d-%H%M%S)"
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║     RESTORE TEST REPORT — BlackDuck $ENV               ║"
echo "║     $TEST_ID                           ║"
echo "╚══════════════════════════════════════════════════════════════╝"
...
if pwsh -Command ". '$PROJECT_ROOT/shared/lib/backup-exec-api.ps1'; Start-BERestore -JobName 'BlackDuck-${ENV}-Daily' -RestorePath '$STG_HOST' $RESTORE_DATE_ARG" 2>/dev/null; then
...
run_test 2 "Service active"     "active"    "ssh_run '$STG_HOST' 'systemctl is-active $BD_SERVICE' 2>/dev/null | grep -q 'active' && echo 'PASS' || echo 'FAIL'"
run_test 3 "Web UI accessible"  "HTTP 200"  "curl -s -k -o /dev/null -w '%{http_code}' 'https://$STG_HOST:$BD_WEB_PORT' 2>/dev/null | grep -q '200' && echo 'PASS' || echo 'FAIL'"
